...
class Stitcher:
    ...
    def stitch(self, images, ratio=0.75, reprojThresh=0.1,showMatches=False):
        ...
        (matches, H, status) = M
        np.save("H" , H)
        #result = self.warp(images ,H )
        #return result
    
    def warp(self ,images , H):
...
if __name__ == "__main__":
    ...
    """
    stitcher.stitch(images,ratio=0.9)
    H = np.load('H.npy')
    while(1):
        start = time()
        result = stitcher.warp(images , H)
